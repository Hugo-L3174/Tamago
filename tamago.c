...
int main()
{
  ...
  /* TODO MULTICORE
  on lance le core1 en mode attente de fonctions et on fait se passer aux cores la config interne du tama: comme ça on
  compute en thread la config, on la passe au core0 qui update les frames intermédiaires et c'est repassé pour les 3
  frames

  mieux: image est partagée entre les cores avec un mutex: mutex_try_enter()
  power down avec  multicore_reset_core1()

  au lieu d'une seule screen image, faire 3 images: une pour le sprite et deux pour les menus haut et bas: peut être
  threadé
...
int tama_init(void)
{
  ...
  tama_.sprite = (sprite){.frames = tamaSprites[randomSpecies],
                          .currentFrame = tamaSprites[randomSpecies][0],
                          .xOrig = 0,
                          .yOrig = 40,
                          .goingRight = true};
}
...
int debug_images(void)
{
  ...
  const unsigned char test[16] = {
      0xf0, 0x00, 0x00, 0x00, 0x23, 0x78, 0xaa, 0xaa, 0xaa,
      0x78, 0x77, 0xaa, 0x21, 0x22, 0xa7, 0xaa /*,
  ...
  const unsigned char test32x16[256] = {
      ...
      ,
  ...
  const unsigned char test17x16[136] = {
      0xf0, 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0, 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0, 0x12, 0x34,
      0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0, 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0, 0x12, 0x34, 0x56, 0x78, 0x9a,
      0xbc, 0xde, 0xf0, 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0, 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0,
      0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0, 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0, 0x12, 0x34, 0x56,
      0x78, 0x9a, 0xbc, 0xde, 0xf0, 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0, 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc,
      0xde, 0xf0, 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0, 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0, 0x12,
  ...
  // 		// explication: x/2 (troncature) parce que l'indice du header ne doit avancer qu'une fois tous les 2 (2 infos en
  // un char)
  // 		// pour y parce que l'indice hauteur est fait une fois toute la largeur de l'image fait (beware if impair)
  // 		// pAddr reste pareil mais x doit être aussi divisé par 2 (troncature ok, adresses de 0 à 64 et on peut shifter
  // ou non pour avoir le bon pixel) 		pAddr=x+(xStart/2)+((y+yStart)*screenWidthBytes); // address of the pixel on the
  // screen (widthbyte depends on the screen itself, has to be 64) 		Paint.Image[pAddr] = (unsigned
  // char)image_buffer[Addr];
