...
void dinoDraw(dinoGame * game, uint8_t ** screenBuffer, const unsigned char ** spriteFramePtr)
{
  Paint_Clear(BLACK);
  // Character
  // XXX find a way to alternate between frames + fixed frame if jumping
  // maybe using score modulo 10 to flip frame or something -> need to add an internal value in game struct to know
  // which frame should be displayed
  Paint_DrawImage_Flipped(spriteFramePtr[leftmouthopenFrame], game->charaPosX_,
                          100 - game->charaHeight_ - game->charaPosZ_, 38, 49);
  ...
  while(currentCact != NULL)
  {
    ...
    {
      Paint_DrawRectangle(currentCact->posX_, 94 - 6 * i, currentCact->posX_ + 6, 100 - 6 * i, WHITE, DOT_PIXEL_1X1,
                          DRAW_FILL_FULL);
    }
...
void dinoLogic(dinoGame * game, volatile bool * buttonPressed)
{
  ...
  while(currentCact != NULL)
  {
    ...
    if(game->charaPosX_ + 28 >= currentCact->posX_ && game->charaPosX_ + 8 <= currentCact->posX_ + 6
       && game->charaPosZ_ <= currentCact->height_ * 6)
    {
      game->gameOver_ = true;
    }
    currentCact = currentCact->next_;
...
// Callback correction: find a way to keep the same callback across everything: button presses must update a global flag
// that should be used by all logics
