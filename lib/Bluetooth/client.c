    ...
    case TC_W4_CHARACTERISTIC_RESULT:
      switch(hci_event_packet_get_type(packet))
      {
        ...
        case GATT_EVENT_QUERY_COMPLETE:
          ...
          gatt_client_listen_for_characteristic_value_updates(&notification_listener, handle_gatt_client_event,
  ...
  switch(event_type)
  {
    case BTSTACK_EVENT_STATE:
      if(btstack_event_state_get_state(packet) == HCI_STATE_WORKING)
      {
        ...
        Paint_DrawString_EN(10, 10, "Addresse locale:", &Font8, 0x1, 0xb);
        Paint_DrawString_EN(10, 30, bd_addr_to_str(local_addr), &Font8, 0x1, 0xb);
    ...
    case GAP_EVENT_ADVERTISING_REPORT:
      ...
      state = TC_W4_CONNECT;
      gap_stop_scan();
      Paint_DrawString_EN(10, 50, "Addresse externe:", &Font8, 0x1, 0xb);
      Paint_DrawString_EN(10, 70, bd_addr_to_str(server_addr), &Font8, 0x1, 0xb);
...
#if 1 // this is only necessary when using polling (which we aren't, but we're showing it is still safe to call in this
      // case)
