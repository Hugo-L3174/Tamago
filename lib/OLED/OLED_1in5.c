...
// flipping bit 4 to 0 disables com remap
  ...
  OLED_WriteReg(0xb1); // set phase length of display phase 1 (discharge of capacitance to reset pixel) and phase 2 (pre
  ...
  OLED_WriteReg(
      0x62); // only first 2 bits should have an effect? enable or not second precharge, and internal or external VSL
...
{
  if((Xstart > OLED_1in5_WIDTH) || (Ystart > OLED_1in5_HEIGHT) || (Xend > OLED_1in5_WIDTH) || (Yend > OLED_1in5_HEIGHT))
    return;
...
void OLED_1in5_Display(UBYTE * Image)
{
  ...
  for(y = 0; y < OLED_1in5_HEIGHT; y++)
    for(x = 0; x < OLED_1in5_WIDTH / 2; x++)
    {
      ...
      temp = Image[x + y * 64]; // writing data for desired pixel, adress is bc Xcoordinate + Ycoord*screen width (rows
                                // completed)
      OLED_WriteData(temp);
    }
}
